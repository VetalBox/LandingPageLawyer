...
$_SESSION['urlpage']="<a href='tovar.php'>Главная</a> \ <a href='tovar.php'>Статьи</a>";
...
if(isset($action)){
    ...
    switch ($action){
        ...
        $delete=mysqli_query($link, "DELETE FROM `table_blog` WHERE id='$id'");
        break;
        }
        else{
            $msgerror='У Вас нет прав доступа на удаление статей';
        }
    }
}
...
<?php
    ...
    $all_count=mysqli_query($link, "SELECT * FROM `table_blog`");
...
<div id="block-info">
<p id="count-style">Всего статей - <strong><?php echo $all_count_result; ?></strong></p>
<p align="right" id="add-style"><a href="add_product.php">Добавить статью</a></p>
</div>
...
$count=mysqli_query($link, "SELECT COUNT(*) FROM `table_blog` $cat");
...
$start=$page*$num-$num;
if($temp[0]>0){
$result=mysqli_query($link, "SELECT * FROM `table_blog` $cat ORDER BY id DESC LIMIT $start, $num");
